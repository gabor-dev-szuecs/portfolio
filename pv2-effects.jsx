...
function toggleTheme(originEl) {
  ...
  const rect = originEl ? originEl.getBoundingClientRect() : { left: window.innerWidth - 40, top: 40, width: 32, height: 32 };
...
function MagnetWrap({ children, strength = 0.25 }) {
  const ref = useRef(null);
  useEffect(() => {
  ...
  return <span ref={ref} style={{ display: 'inline-block', transition: 'transform .35s cubic-bezier(.2,.7,.2,1)' }}>{children}</span>;
}

Object.assign(window, { useReveal, CursorRing, useScrolled, toggleTheme, MagnetWrap });
